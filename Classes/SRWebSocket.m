...
- (OSStatus)extractIdentity:(CFDataRef)inP12Data :(SecIdentityRef*)identity {
    ...
    CFStringRef pStr = CFSTR("like1982");
    const void *keys[] = { kSecImportExportPassphrase };
    const void *values[] = { pStr };
...
- (void)_updateSecureStreamOptions;
{
    if (_secure) {
        ...
        NSArray *identityArray = [_urlRequest SR_SSLSecIdentities];
        if (identityArray) {
            [SSLOptions setObject:identityArray forKey:(__bridge id)kCFStreamSSLCertificates];
            [SSLOptions setValue:@NO forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
            [SSLOptions setObject:@NO forKey:(__bridge id)kCFStreamSSLIsServer];
            [SSLOptions setObject:(__bridge id)kCFStreamSocketSecurityLevelNegotiatedSSL forKey:(__bridge id)kCFStreamSSLLevel];
        }
...
- (NSArray *)SR_SSLSecIdentities;
{
    return [NSURLProtocol propertyForKey:@"SR_SSLSecIdentities" inRequest:self];
}
...
- (void)setSR_SSLSecIdentities:(NSArray *)SR_SSLSecIdentities;
{
    [NSURLProtocol setProperty:SR_SSLSecIdentities forKey:@"SR_SSLSecIdentities" inRequest:self];
}
